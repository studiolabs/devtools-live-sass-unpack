...
SassUnpack.prototype.unpack = function() {
  ...
  var SassMap = require("devtools-live-sass-map");
...
SassUnpack.prototype.cutSourceFileWithMap = function(srcFilePath, mapSass) {
  ...
  var loadPaths = _([this.sourceDir])
  ...
  content.split("\n").forEach(
    function(line, iLine) {
      ...
      if (matchImport) {
        var importFilePath = resolveSassPath(
          matchImport[1].trim(),
          loadPaths,
          this.extensions
        );
        ...
        if (importFilePath) {
          ...
          if (
            mapSass.index[this.filepath].dependencies.indexOf(importFilePath) >=
            0
          ) {
          ...
          throw new Error(
            "Impossible to resolve Sass Path  in " +
              this.filepath +
              " - line : " +
              iLine +
              " \n " +
...
SassUnpack.prototype.generateFiles = function(files, mapSass, toPath) {
  ...
  if (this.bMap) {
    this.write(
      toPath + "/" + this.name + "/sass.map.json",
      JSON.stringify(map)
    );
  ...
  if (this.bHref) {
    this.write(
      toPath + "/" + this.name + "/sass.links.json",
